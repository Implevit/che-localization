report 50105 "EIN Standard Sales - Shipm501"
{
    ...
    dataset
    {
        dataitem(Header; "Sales Shipment Header")
        {
            ...
            dataitem(Line; "Sales Shipment Line")
            {
                ...
                column(Pos_Line; Line.Position)
                {
                }
                //EIN++
                column(ShowSalesShipmentLine; ShowSalesShipmentLine)
                {
                }
                ...
                column(DeliveryDate_Line_Lbl; PostedDeliveryDateLbl)
                {
                }
                column(Cross_Reference_No_; CrossReferenceNoLbl)
                {

                }

                column(Quantity_Line_SL; FormattedQuantity_SL)
                {
                }
                column(Quantity_Line_Lbl_SL; Quantitylbl_SL)
                {
                }

                column(Unitprice_Line_SL; FormattedUnitprice_SL)
                {
                }
                column(Unitprice_Line_Lbl_SL; Unitpricelbl_SL)
                {
                }

                column(Discount_Line_SL; FormattedDiscount_SL)
                {
                }
                column(Discount_Line_Lbl_SL; Discountlbl_SL)
                {
                }

                column(Amount_Line_SL; FormattedAmount_SL)
                {
                }
                column(Amount_Line_Lbl_SL; Amountlbl_SL)
                {
                }
                ...
                begin
                    ...
                    if HasTypeToFillMandatoryFields() then begin
                        ...
                        FormattedQuantity_SL := format(l_SalesLine.Quantity);
                        FormattedUnitPrice_SL := format(l_SalesLine."Unit Price");
                        FormattedDiscount_SL := format(l_SalesLine."Line Discount %");
                        FormattedAmount_SL := format(l_SalesLine.amount);

                        Unitpricelbl_SL := l_SalesLine.FieldCaption("Unit Price");
                        Discountlbl_SL := l_SalesLine.FieldCaption("Line Discount %");
                        Amountlbl_SL := l_SalesLine.FieldCaption(Amount);



                    end else begin
                        FormattedQuantity := '';
                        FormattedQuantity_SL := '';
                        FormattedUnitPrice_SL := '';
                        FormattedDiscount_SL := '';
                        FormattedAmount_SL := '';
                    end;
                    //ORI                    
                    //if HasTypeToFillMandatoryFields() then begin
            ...
            begin
                ...
                IF ShippingAgent.GET(Header."Shipping Agent Code") THEN
                    IF ShippingAgent.Name <> '' THEN
                        ShippingAgentText := Text76702 ELSE
                        ShippingAgentText := '';
                //EIN--                
    ...
    var
        ...
        ItemReference: Record "Item Reference"; //EIN
        ShippingAgent: Record "Shipping Agent";//EIN
        ...
        ShowLotSN: Boolean;
        ShowSalesShipmentLine: Boolean; //EIN
        TariffNo: Text[50];//EIN
        EAN: Text[60];//EIN
        ShippingAgentText: Text[100];//EIN

        FormattedQuantity_SL: Text;//EIN
        FormattedUnitPrice_SL: Text;//EIN
        FormattedDiscount_SL: Text;//EIN
        FormattedAmount_SL: Text;//EIN
    ...
    begin
        ...
        FillNameValueTable(RightHeader, Header.Fieldcaption("Sell-to Customer No."), Header."Sell-to Customer No."); //ok
        FillNameValueTable(RightHeader, Header.FieldCaption("External Document No."), Header."External Document No.");//ok
        FillNameValueTable(RightHeader, Header.FieldCaption("Order No."), Header."Order No."); //ok
        FillNameValueTable(RightHeader, Header.FieldCaption("VAT Registration No."), Header."VAT Registration No."); //ok
        FillNameValueTable(RightHeader, Header.FieldCaption("Order Date"), Format(Header."Order Date", 0, '<Day>/<Month>/<YEAR>'));
        FillNameValueTable(RightHeader, PostedDeliveryDateLbl, Format(Header."Requested Delivery Date", 0, '<Day>/<Month>/<YEAR>'));

        //FillNameValueTable(RightHeader, DeliveryNoLbl, Header."No.");
